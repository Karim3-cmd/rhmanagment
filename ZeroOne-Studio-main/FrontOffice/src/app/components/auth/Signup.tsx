      ...
      <div className="w-full max-w-md">
        <div className="bg-white rounded-lg shadow-lg p-8">
          ...
          <form onSubmit={handleSubmit}>
            <div className="mb-6">
              <label className="block text-sm mb-2 text-gray-700">Register As</label>
              <div className="flex gap-2">{(['Employee', 'Manager', 'HR'] as UserRole[]).map((r) => <button key={r} type="button" onClick={() => setRole(r)} className={`flex-1 py-2 px-4 rounded-lg border transition-all ${role === r ? 'bg-primary text-white border-primary' : 'bg-white text-gray-700 border-input hover:border-primary'}`}>{r}</button>)}</div>
            </div>
            <div className="mb-4">
            ...
            <div className="mb-4">
              ...
              {errors.password && <div className="flex items-center gap-1 mt-1 text-destructive text-sm"><AlertCircle className="w-4 h-4" /><span>{errors.password}</span></div>}
            </div>
            <div className="mb-6">
              ...
              {errors.confirmPassword && <div className="flex items-center gap-1 mt-1 text-destructive text-sm"><AlertCircle className="w-4 h-4" /><span>{errors.confirmPassword}</span></div>}
            </div>
            {errors.submit && <div className="mb-4 rounded-lg bg-red-50 p-3 text-sm text-red-700">{errors.submit}</div>}
