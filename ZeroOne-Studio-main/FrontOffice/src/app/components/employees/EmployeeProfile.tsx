...
export function EmployeeProfile({ employeeId, onBack, userRole }: EmployeeProfileProps) {
  ...
  const getLevelLabel = (level: number) => level >= 5 ? 'Expert' : level >= 4 ? 'Advanced' : level >= 3 ? 'Good' : level >= 2 ? 'Basic' : 'Beginner';
  const getLevelColor = (level: number) => level >= 5 ? 'bg-green-100 text-green-700' : level >= 4 ? 'bg-blue-100 text-blue-700' : level >= 3 ? 'bg-yellow-100 text-yellow-700' : level >= 2 ? 'bg-orange-100 text-orange-700' : 'bg-red-100 text-red-700';
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm p-6 border border-border"><h2 className="text-2xl mb-6 text-gray-900">Skills & Competencies</h2><div className="space-y-4">{skills.length === 0 ? <div className="text-muted-foreground">No skills assigned yet.</div> : skills.map((skill) => { const level = skill.assignments?.find((assignment) => assignment.employeeId === employeeId)?.level || 0; return <div key={skill._id} className="border border-border rounded-lg p-4"><div className="flex items-center justify-between mb-3"><div className="flex items-center gap-3"><h3 className="font-medium text-gray-900">{skill.name}</h3><span className={`text-xs px-2 py-1 rounded-full ${getTypeColor(skill.type)}`}>{skill.type}</span><span className={`text-xs px-2 py-1 rounded-full ${getLevelColor(level)}`}>{getLevelLabel(level)}</span></div><div className="flex items-center gap-2"><StarRating value={level} readonly /><span className="text-sm text-muted-foreground">{level}/5</span></div></div><div className="w-full bg-secondary rounded-full h-2"><div className="bg-primary h-2 rounded-full transition-all" style={{ width: `${(level / 5) * 100}%` }} /></div></div>; })}</div><div className="mt-4 text-sm text-muted-foreground">Average assigned skill rating: {averageSkillLevel}/5</div></div>
